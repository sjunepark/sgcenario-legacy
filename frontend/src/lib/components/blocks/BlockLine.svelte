<script lang="ts">
	...
	export let character = "박세준입니다아아아";
...
{#if $open}
	<ul
		class="prose z-10 max-h-48 overflow-y-auto bg-stone-50 shadow-lg ring-1 ring-stone-500/50"
		{...$menu}
		use:menu
	...
	>
		...
		{#each filteredCharacters as character, index (index)}
			<li
				...
				use:option
				class="not-prose relative cursor-pointer scroll-my-2 data-[highlighted]:bg-stone-200 data-[disabled]:opacity-50"
