...
export const d1Adapter = (db, tables) => {
        ...
        return {
            ...
            setUser: async (user, key) => {
                ...
                const insertUserStatement = db
                    .prepare(`INSERT INTO ${ESCAPED_USER_TABLE_NAME} ( ${userFields} ) VALUES ( ${userValues} )`)
                    .bind(...userArgs);
                if (!key) {
                ...
                try {
                    ...
                    const insertKeyStatement = db
                        .prepare(`INSERT INTO ${ESCAPED_KEY_TABLE_NAME} ( ${keyFields} ) VALUES ( ${keyValues} )`)
                        .bind(...keyArgs);
                    await db.batch([insertUserStatement, insertKeyStatement]);
                }
                catch (e) {
                    const error = e;
                    if (error.cause?.message?.includes("UNIQUE constraint failed") &&
            ...
            updateUser: async (userId, partialUser) => {
                ...
                await db
                    .prepare(`UPDATE ${ESCAPED_USER_TABLE_NAME} SET ${getSetArgs(fields, values)} WHERE id = ?`)
                    .bind(...args, userId)
                    .run();
            },
            ...
            setSession: async (session) => {
                ...
                try {
                    ...
                    await db
                        .prepare(`INSERT INTO ${ESCAPED_SESSION_TABLE_NAME} ( ${fields} ) VALUES ( ${values} )`)
                        .bind(...args)
                        .run();
                }
                catch (e) {
            ...
            deleteSessionsByUserId: async (userId) => {
                ...
                await db
                    .prepare(`DELETE FROM ${ESCAPED_SESSION_TABLE_NAME} WHERE user_id = ?`)
                    .bind(userId)
                    .run();
            },
            updateSession: async (sessionId, partialSession) => {
                ...
                await db
                    .prepare(`UPDATE ${ESCAPED_SESSION_TABLE_NAME} SET ${getSetArgs(fields, values)} WHERE id = ?`)
                    .bind(...args, sessionId)
                    .run();
            },
            getKey: async (keyId) => {
            ...
            setKey: async (key) => {
                try {
                    ...
                    await db
                        .prepare(`INSERT INTO ${ESCAPED_KEY_TABLE_NAME} ( ${fields} ) VALUES ( ${values} )`)
                        .bind(...args)
                        .run();
                }
                catch (e) {
            ...
            updateKey: async (keyId, partialKey) => {
                ...
                await db
                    .prepare(`UPDATE ${ESCAPED_KEY_TABLE_NAME} SET ${getSetArgs(fields, values)} WHERE id = ?`)
                    .bind(...args, keyId)
                    .run();
            },
            getSessionAndUser: async (sessionId) => {
                ...
                const getUserFromJoinStatement = db
                    .prepare(`SELECT ${ESCAPED_USER_TABLE_NAME}.*, ${ESCAPED_SESSION_TABLE_NAME}.id as __session_id FROM ${ESCAPED_SESSION_TABLE_NAME} INNER JOIN ${ESCAPED_USER_TABLE_NAME} ON ${ESCAPED_USER_TABLE_NAME}.id = ${ESCAPED_SESSION_TABLE_NAME}.user_id WHERE ${ESCAPED_SESSION_TABLE_NAME}.id = ?`)
                    .bind(sessionId);
                const [{ results: sessionResults }, { results: userFromJoinResults }] = (await db.batch([
                    getSessionStatement,
